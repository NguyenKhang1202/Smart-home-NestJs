...
function DetailRoom(props) {
  ...
  useEffect(() => {
    axios.get(`/api/v1/devices`, { params: { roomId: roomId } }).then((res) => {
      let light = [];
  ...
  useEffect(() => {
    onValue(ref(database, 'sensors/'), (snapshot) => {
      ...
      const newDataSensor = arr[arr.length - 1];
      const dataSensor = newDataSensor.data;
  ...
  const onChange = (id, checked, event) => {
    ...
    axios
      ...
      .then((res) => {
        ...
        if (airCondition && (id === airCondition._id || id === airCondition.id)) {
  ...
  const handleLock = () => {
    ...
    axios
      .put(`/api/v1/devices/${id}`, { status: tmp })
      .then((res) => {
              ...
              {airCondition === undefined ? (
                ...
                >
                  <Switch
                    ...
                    onChange={(checked, event) => onChange(airCondition._id || airCondition.id, checked, event)}
                  />
                ...
                <Col span={6} key={index}>
                  <Card hoverable cover={<GiLightBulb size={54} color={item.status === 'on' ? '#ece707' : 'black'} />}>
                    <Switch
                      ...
                      onChange={(checked, event) => onChange(item._id || item.id, checked, event)}
                    />
